...
由于 nginx 是第7层(Application Layer In [OSI network model](https://en.wikipedia.org/wiki/OSI_model))的负载均衡器，虽说 nginx 有 TCP 插件，可以转发第3层（Network Layer）的 TCP 连接，但 Ingress-nginx 对 [TCP 请求的转发规则的支持](https://github.com/kubernetes/ingress-nginx/blob/master/docs/user-guide/exposing-tcp-udp-services.md) 可能不能满足我们的需求。

假设我们有三个环境——Production, Staging 和 Test，三个环境下都有一个 tcp 服务，在 443 端口提供服务。它们域名分别为:
...
那么 nginx 就能满足我们3个环境的需求了，但目前这只是我的一个设想，会找时间询问 Kubernetes 社区，并发起 pull request。

在上述想法还未实现的情况下，针对这3个环境下的 tcp 服务，类似于 ingress nginx 的方式，可以单独通过 LoadBalancer 类型的 Service 对外暴露，获取到外部 IP 之后，再手动设置 DNS 解析。
